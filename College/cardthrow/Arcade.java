...
public class Arcade extends JPanel implements ActionListener {
	...
	public void fixImage(JButton b)
	{
		Rectangle r=b.getBounds();
		ImageIcon ii=(ImageIcon)b.getIcon();
		int h=ii.getIconHeight(),w=ii.getIconWidth();
		...
		if(change)
		{
			ii.setImage(ii.getImage().getScaledInstance(w, h, Image.SCALE_REPLICATE));
		}
	}
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		catch(Exception e2)
		{
			...
			try
			{
				...
				URLClassLoader ucl = new URLClassLoader(url);
				Class c = ucl.loadClass(str.substring(i,str.length()-6).replaceAll("%5C", ""));
				Game g = (Game)c.newInstance();
				frame.setVisible(false);
				frame.setContentPane(g);
				frame.setJMenuBar(g.createMenuBar());
