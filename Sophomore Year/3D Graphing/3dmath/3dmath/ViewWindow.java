...
public class ViewWindow {
    ...
    */
    public void project(Points v)
    {
        // project to view window
        double p=distanceToCamera / (v.z+panz);
        v.x2d =p * v.x;
        v.y2d =p * v.y;

        // convert to screen coordinates
        v.x2d = convertFromViewXToScreenX(v.x2d);
        v.y2d = convertFromViewYToScreenY(v.y2d);
    }
	...
	public double unjectX(double x2d,double z)
	{
		x2d=convertFromScreenXToViewX(x2d);
		return x2d*(z+panz)/distanceToCamera;
	}
	public double unjectY(double y2d,double z)
	{
		y2d=convertFromScreenYToViewY(y2d);
		return y2d*(z+panz)/distanceToCamera;
	}
}
